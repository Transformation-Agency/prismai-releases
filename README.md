...
Windows `0.5.0` is currently available as a manual `.exe` installer. The Windows updater bundle (`.nsis.zip` plus `.sig`) is generated by the Windows release workflow and should be added to this release plus `update.json` before enabling in-app Windows auto-update.
